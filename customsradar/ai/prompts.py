...
（一）公司画像 （二）利弊评估（优势/劣势/机会/风险） （三）切入点分析
（四）真实联系人 （五）优先级判断。
...
利弊评估要站在「我们能不能拿下这单」的角度，不是泛泛的 SWOT 教科书：
- strengths：他们哪里强 → 对我们意味着什么样的机会（比如采购量大 = 值得深耕）
- weaknesses：他们哪里弱 → 我们能不能补上（比如供应商单一 = 我们做备选）
- opportunities：具体怎么切进去
- threats：追这家的风险（可能已有长期供应商、可能是同行套价、付款/政策风险）

contacts_found：只填官网/材料里**真实出现**的人名和职位，一个都编不得 ——
这些会拿去推测邮箱，编错就把开发信发给不存在的人。没有就留空数组。
